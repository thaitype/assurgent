...
# Run the bot
cd app && bun run dev    # uses --env-file .env
...
A Telegram bot wrapper that connects Telegram to the Claude Code CLI. It forwards user messages to `claude -p`, manages conversation sessions, and returns responses. It is not an agent -- it is a thin bridge so you can talk to Claude Code from your phone.
...
- Config: config.json + `.env` via Bun `--env-file`

### Key Architectural Patterns

- **Adapter pattern.** `ChatAdapter` for chat platforms, `AgentAdapter` for CLI agents. Pluggable interfaces.
- **Core wrapper.** Routes messages between chat and agent adapters. Does not duplicate agent logic.
- **Session manager.** Maps friendly session names to agent session IDs. Handles turn limits.
- **Per-chat message queue.** Prevents concurrent agent invocations for the same chat.

### Directory Structure

```
app/
├── .chief/                  # Chief Agent Framework (planning, rules, goals)
├── src/
│   ├── index.ts             # Entry point -- load config, wire adapters, start
│   ├── config.ts            # Config type + loader + validation
│   ├── core/
│   │   ├── wrapper.ts       # Core orchestrator -- routes messages
│   │   └── session-manager.ts  # Session name <-> UUID mapping
│   ├── interfaces/
│   │   ├── chat-adapter.ts  # ChatAdapter interface
│   │   └── agent-adapter.ts # AgentAdapter interface
│   ├── chat/
│   │   └── telegram.ts      # ChatAdapter impl: Telegram (grammy)
│   ├── agent/
│   │   └── claude-code.ts   # AgentAdapter impl: Claude Code CLI
│   ├── secrets/
│   │   ├── provider.ts      # SecretProvider interface
│   │   ├── resolver.ts      # ${{secretRef.*}} handlebar resolution
│   │   ├── azure-keyvault.ts # Azure Key Vault adapter (with retry)
│   │   └── env.ts           # Env var provider (reads process.env)
│   └── proxy/
│       └── proxy.ts         # Generic secret proxy (resolves handlebars in requests)
├── config.json              # Runtime config (gitignored, has secrets)
...
- **`.env` loaded via Bun `--env-file`.** The `env` secret provider reads `process.env`. No custom `.env` parsing -- Bun handles it. Config structure lives in `config.json`; secrets reference providers.
- **Workspace is the assurgent repo.** Claude Code runs with cwd set to `assurgent/`, not an isolated workspace.
